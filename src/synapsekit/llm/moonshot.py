...
class MoonshotLLM(BaseLLM):
    """Moonshot AI (Kimi) LLM provider with async streaming.

    Uses the OpenAI-compatible API. Models include ``moonshot-v1-8k``,
    ``moonshot-v1-32k``, ``moonshot-v1-128k``.
    """

    def __init__(self, config: LLMConfig, base_url: str | None = None) -> None:
        super().__init__(config)
        self._base_url = base_url or _MOONSHOT_BASE_URL
